...
The repository now has a working portable core scaffold, a fixed-step simulation shell with explicit shot phases, cue-strike input seeding for velocity and spin, a cloth-motion layer for straight-line travel, equal-mass ball-ball collision resolution, hardcoded cushion and pocket-jaw boundary interaction, pocket capture with shot-event emission, deterministic replay trace capture with a locked straight-shot regression signature, a standard 8-ball opening rack factory, a portable 8-ball rules layer, a portable training-mode rules path, a portable training-scenario library with built-in presets, and a Godot 4.6 adapter that records shot traces, resolves them through the portable rules engines, exposes rules/training state in the HUD, renders predictive guide lines from cloned portable simulations, can show the hardcoded table geometry as a toggleable overlay, and can load the portable practice presets directly in training mode. The next implementation step remains broader presentation polish and scenario tooling.
...
- 2026-03-14: Step 14 continued. Added a portable `TrainingScenarioLibrary` with six built-in practice presets, standalone coverage for layout legality and suggested shots, and Godot training-mode hotkeys (`1` through `6`) that load those presets with scenario-specific aim, speed, and tip defaults. Reverified with `dotnet test` for `41/41` passing standalone tests, `dotnet build` for the Godot adapter, Godot 4.6 Mono `--build-solutions`, and a clean headless startup run via `--quit-after 10`.
