...
The repository now has a working portable core scaffold, a fixed-step simulation shell with explicit shot phases, cue-strike input seeding for velocity and spin, a cloth-motion layer with skid-to-roll matching plus modest side-spin drift/scrub, equal-mass ball-ball collision resolution with tangential spin transfer, hardcoded cushion and pocket-jaw boundary interaction with tangential rail response, a mouth/drop-based pocket capture model derived from hardcoded jaw geometry, deterministic replay trace capture with a locked straight-shot regression signature, a standard 8-ball opening rack factory, a portable 8-ball rules layer, a portable training-mode rules path, and a Godot 4.6 adapter that records shot traces, resolves them through the portable rules engines, exposes rules/training state in framed HUD panels, renders predictive guide lines from cloned portable simulations, can now show the hardcoded table geometry as a toggleable overlay with per-layer visibility controls, boots into an orthographic top-down main camera while still supporting switchable camera presets with runtime zoom for inspection, directly instantiates the checked-in Blender table asset at `res://art/customtable_9ft.blend` when present, shows transient shot-feedback banners for key events and rule outcomes, highlights current turn state in a dedicated color-accented header, keeps a dedicated last-shot summary panel for completed eight-ball and training/freeplay shots, marks the selected training/freeplay ball with a pulsing in-world ring during placement, now exposes live debug tuning for key portable-physics constants, uses a cleaner card-based HUD with a dedicated shot-setup panel and toggleable controls/help panel, opens on a proper start/menu overlay with button-based mode selection and pause/menu actions, and runs Player 2 as a simple computer opponent in 8-ball while leaving freeplay human-controlled. The next implementation step is rail rebound tuning and richer english on rails.
