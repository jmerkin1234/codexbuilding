...
- The `Tuning` window now also keeps a fixed legend panel on the right side with color swatches and plain-language labels for cloth bounds, cushions, jaws, pocket capture, cue/rack spots, the three aim guides, and the pale-yellow selected-target highlight, so overlay colors can be read without relying on the gameplay HUD text.
