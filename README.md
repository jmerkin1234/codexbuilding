...
- `Tuning` mode keeps the hardcoded overlay visible by default, opens a separate movable play-mode `Window`, highlights the currently selected calibration target, and now uses a flat scrollable slider list with one row per tunable field plus Save/Reload/Reset controls instead of relying on adjustment hotkeys or a cramped grouped HUD block.
- The `Tuning` window now keeps all rows visible at once and uses its dropdown only to jump to and highlight one object, so rails, jaws, pockets, and spots can be tuned per object without collapsing the rest of the calibration list.
