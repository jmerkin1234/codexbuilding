...
- `Tuning` mode keeps the hardcoded overlay visible by default, highlights the currently selected calibration target, and now uses HUD dropdowns, sliders, and buttons for field selection, value adjustment, overlay thickness, and save/reload/reset instead of relying on adjustment hotkeys.
