...
- standalone `Tuning` mode for persistent hardcoded-table calibration
...
Right now the repo is in step 14. The Godot adapter is rules-aware, training-aware, has predictive guide visuals, exposes the hardcoded table geometry as a toggleable overlay with per-layer visibility control, now boots into an orthographic top-down main camera while still supporting switchable inspection camera presets with zoom, presents a cleaner HUD split across status, shot-summary, shot-setup, controls/help, and a detached debug window, now lets `F7` hide the gameplay HUD and shot banner entirely without affecting the menu overlay or the separate tuning window, opens on a proper start/menu overlay for `EightBall`, `FreePlay`, and `Tuning` mode selection, shows transient shot-feedback banners, highlights turn state in a color-accented header, now keeps a dedicated last-shot summary panel for completed eight-ball and training/freeplay shots, directly loads the checked-in Blender table asset through Godot’s `.blend` importer, now uses the authored Blender balls and cue stick instead of procedural fallback ball visuals, now keeps the imported cue stick tip about `0.03 m` off the cue ball during shot setup by deriving the cue tip offset from the mesh geometry, now accumulates visible ball roll from mirrored core motion plus side-spin yaw so the balls no longer read like sliding decals, now imports the ball textures lossless with mipmaps disabled and keeps 3D rendering at native scale with TAA/screen-space AA disabled plus `MSAA 3D` enabled for a sharper standalone view, now shows shot power as a percentage of the active strike-speed cap, now clamps player shot setup to `0.3-5.0 m/s` for regular play and `0.3-8.0 m/s` on the eight-ball break, now fails a bad computer turn forward instead of soft-locking by handing the opponent ball in hand, now runs Player 2 as a simple computer opponent in 8-ball with separate regular-shot and break-shot speed samples while leaving freeplay human-only, and now renders the white, cyan, and target-path aim guides at a true `2 px` screen thickness with a longer post-contact preview span. Step 14 now also includes a standalone `Tuning` mode that persists `user://table_calibration.json`, rebuilds `TableSpec` from the hardcoded source plus calibration offsets, keeps the overlay visible, hides the main HUD cards while tuning, opens a separate movable play-mode tuning window, keeps all tuning rows visible in one flat scrollable list, uses its dropdown only to jump to and highlight one object, adds direct selected-object mini-panels for literal X/Y/Angle-style edits in addition to the flat list and the direct angle fields for every cushion and jaw segment, now keeps a fixed right-side legend for the overlay and aim-guide colors, now uses explicit always-visible tuning scrollbars, and now expresses overlay thickness in pixels with a `0.5 px` to `2.0 px` range. The Godot-side code is also now split into focused partial files so `Main.cs` stays as lifecycle/shared-state glue instead of growing back into the whole adapter. The portable core also now has richer side-spin behavior on cloth, tangential spin transfer in ball-ball collisions, a mouth/drop-based pocket capture model derived from hardcoded jaw geometry plus a first shelf-like slow lip-hang rejection pass, angle-aware rail rebound tuning with separate glancing/head-on restitution and retained bank-speed control, a separate rail-english transfer term that makes opposite english produce meaningfully different bank outcomes, object-contact follow/draw carry-through that keeps cue-ball top/back spin meaningful after impact, stable sorted first-contact selection for simultaneous cue-ball contacts, and overlap correction that no longer emits fake rail or ball-contact events into the rules layer. The remaining backlog is resyncing stale hardcoded geometry from the latest Blender source, broader pocket-behavior and overall feel tuning, stronger AI, and broader presentation polish.
